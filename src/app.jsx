        ...
        const {
            ...
            Pie, Cell, ComposedChart, ReferenceLine
        } = Recharts;
        ...
        function calculateModel(params, trajectories) {
          ...
          for (let i = 0; i < 11; i++) {
            ...
            const virtualExpenses = virtualStudents * (
              BASE_ASSUMPTIONS.virtual.headcount +
              BASE_ASSUMPTIONS.virtual.programs +
              BASE_ASSUMPTIONS.virtual.misc +
            ...
            const microStudents = Math.round(
              existingMicroSchools * BASE_ASSUMPTIONS.micro.studentsPerSchool +
              newMicroSchools * BASE_ASSUMPTIONS.micro.fillRateYr1 * BASE_ASSUMPTIONS.micro.studentsPerSchool
            );
            const microRevenue = microStudents * BASE_ASSUMPTIONS.micro.tuition;
            const microExpenses = microStudents * (
              BASE_ASSUMPTIONS.micro.coachSalary +
              BASE_ASSUMPTIONS.micro.realEstate +
              BASE_ASSUMPTIONS.micro.lifeSkills +
              BASE_ASSUMPTIONS.micro.misc +
            ...
            const studentsPerMidSized =
              BASE_ASSUMPTIONS.midSized.studentsRenovated * BASE_ASSUMPTIONS.midSized.blendRatio +
              BASE_ASSUMPTIONS.midSized.studentsNewBuild * (1 - BASE_ASSUMPTIONS.midSized.blendRatio);
            ...
            const midSizedCapexPerSchool =
              (BASE_ASSUMPTIONS.midSized.purchaseRenovated + BASE_ASSUMPTIONS.midSized.capexRenovated) * BASE_ASSUMPTIONS.midSized.blendRatio +
            ...
            years.push({
              ...
              totalCapex,
              netCashFlow,
              cumulativeCashFlow: 0, // Will calculate below
        ...
        function runSensitivity(params, trajectories, paramName, range) {
          ...
          for (let i = 0; i < steps; i++) {
            ...
            const testParams = { ...params, [paramName]: params[paramName] * mult };
        ...
        function goalSeek(params, trajectories, targetMetric, targetValue, paramToAdjust, tolerance = 0.01) {
          ...
          while (iterations < maxIterations && (high - low) > tolerance) {
            ...
            const testParams = { ...params, [paramToAdjust]: mid };
            const model = calculateModel(testParams, trajectories);
        ...
        // Components
        function MetricCard({ title, value, subValue, icon, color = 'blue' }) {
          const colorClasses = {
        ...
        function SliderInput({ label, value, onChange, min, max, step = 0.1, format = 'percent' }) {
          const displayValue = format === 'percent' ? `${(value * 100).toFixed(0)}%` :
                               format === 'currency' ? formatCurrency(value, true) :
                               format === 'multiple' ? `${value.toFixed(1)}x` : value;
        ...
        function GoalSeekPanel({ params, onApplyResult }) {
          ...
          return (
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6 border border-indigo-200">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Find the required growth rate to achieve a specific target outcome.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const multiplier = e.target.value.includes('B') ? 1e9 :
                                        e.target.value.includes('M') ? 1e6 :
                                        e.target.value.includes('K') ? 1e3 : 1;
                      setTargetValue(parseFloat(val) * multiplier || 0);
                    }}
                    className="w-full p-2 border rounded-lg"
                  />
                  <div className="flex gap-2 mt-1">
                    {[1e9, 3e9, 5e9, 10e9].map(v => (
                      <button
                        key={v}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Parameter to Adjust</label>
                  <select
                    value={paramToAdjust}
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  ...
                  {isCalculating ? 'Calculating...' : 'Calculate Required Value'}
                </button>

                {result !== null && (
                  <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-lg border">
                    <div>
                      <span className="text-sm text-gray-600">Required {paramLabels[paramToAdjust]}:</span>
                      <span className="ml-2 font-bold text-indigo-600">
                        {paramToAdjust.includes('Multiple') ? `${result.toFixed(1)}x` : `${(result * 100).toFixed(0)}%`}
                      </span>
                    </div>
                    <button
        ...
        function UnitEconomicsPanel({ model }) {
          ...
          const tierData = {
            virtual: {
              ...
              costs: [
                { name: 'Headcount', value: BASE_ASSUMPTIONS.virtual.headcount },
            ...
            micro: {
              ...
              costs: [
                { name: 'Coach Salary', value: BASE_ASSUMPTIONS.micro.coachSalary },
                { name: 'Real Estate', value: BASE_ASSUMPTIONS.micro.realEstate },
            ...
            <div className="space-y-6">
              <div className="flex gap-2 flex-wrap">
                {Object.entries(tierData).map(([key, data]) => (
                  <button
                    ...
                    style={selectedTier === key ? { backgroundColor: data.color } : {}}
                  >
                    {data.name}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-xl border p-6">
                  <h4 className="font-semibold text-gray-900 mb-4">Per-Student Unit Economics</h4>

                  <div className="space-y-3">
                    <div className="flex justify-between items-center py-2 border-b border-gray-100">
                      <span className="font-medium text-gray-900">Tuition Revenue</span>
                      <span className="font-bold text-green-600">{formatCurrency(tier.tuition)}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center py-3 bg-gray-50 rounded-lg px-3 mt-2">
                      <span className="font-bold text-gray-900">Contribution Margin</span>
                ...
                {/* CAC/LTV Analysis */}
                <div className="bg-white rounded-xl border p-6">
                  <h4 className="font-semibold text-gray-900 mb-4">CAC / LTV Analysis</h4>

                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="bg-red-50 rounded-lg p-4 text-center">
                      <p className="text-sm text-red-700 mb-1">Customer Acquisition Cost</p>
                      <p className="text-2xl font-bold text-red-900">{formatCurrency(cac)}</p>
                    </div>
                    <div className="bg-green-50 rounded-lg p-4 text-center">
                      <p className="text-sm text-green-700 mb-1">Lifetime Value</p>
                      <p className="text-2xl font-bold text-green-900">{formatCurrency(ltv)}</p>
                    </div>
                  </div>

                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 text-center">
                    <p className="text-sm text-blue-700 mb-1">LTV:CAC Ratio</p>
                    <p className="text-3xl font-bold text-blue-900">{ltvCacRatio}x</p>
                    <p className="text-xs text-blue-600 mt-1">
                      {ltvCacRatio >= 3 ? '✓ Healthy (>3x)' : ltvCacRatio >= 2 ? '⚠ Acceptable' : '✗ Needs improvement'}
                    </p>
                  </div>

                  <div className="mt-4 text-sm text-gray-500">
                    <p>Avg. Student Life: {avgLife} years</p>
                    <p>Payback Period: {(cac / margin).toFixed(1)} years</p>
                  </div>
                </div>
              </div>
              ...
              {/* Tier Summary */}
              <div className="bg-white rounded-xl border p-6">
                <h4 className="font-semibold text-gray-900 mb-4">Year 10 {tier.name} Summary</h4>
                <div className="grid grid-cols-4 gap-4">
                  <div className="text-center">
                    <p className="text-sm text-gray-600">Students</p>
                    <p className="text-xl font-bold">{formatNumber(tier.students, true)}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm text-gray-600">Revenue</p>
                    <p className="text-xl font-bold text-green-600">{formatCurrency(tier.revenue, true)}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm text-gray-600">EBITDA</p>
                    <p className="text-xl font-bold" style={{ color: tier.color }}>{formatCurrency(tier.ebitda, true)}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm text-gray-600">EBITDA Margin</p>
                    <p className="text-xl font-bold">{tier.revenue > 0 ? (tier.ebitda / tier.revenue * 100).toFixed(1) : 0}%</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        function DataTableView({ model, viewMode = 'students' }) {
          ...
          const columns = {
            students: [
              ...
              { key: 'virtualStudents', label: 'Virtual', format: v => formatNumber(v, true) },
              { key: 'microStudents', label: 'Micro', format: v => formatNumber(v, true) },
              { key: 'midSizedStudents', label: 'Mid-Sized', format: v => formatNumber(v, true) },
              { key: 'flagshipStudents', label: 'Flagship', format: v => formatNumber(v, true) },
              { key: 'totalStudents', label: 'Total', format: v => formatNumber(v, true), bold: true },
            ],
            revenue: [
              { key: 'year', label: 'Year', format: v => v },
              { key: 'virtualRevenue', label: 'Virtual', format: v => formatCurrency(v, true), color: 'text-blue-600' },
              { key: 'microRevenue', label: 'Micro', format: v => formatCurrency(v, true), color: 'text-emerald-600' },
              { key: 'midSizedRevenue', label: 'Mid-Sized', format: v => formatCurrency(v, true), color: 'text-amber-600' },
              { key: 'flagshipRevenue', label: 'Flagship', format: v => formatCurrency(v, true), color: 'text-purple-600' },
              { key: 'totalRevenue', label: 'Total', format: v => formatCurrency(v, true), bold: true },
            ],
            ebitda: [
              { key: 'year', label: 'Year', format: v => v },
              { key: 'virtualEBITDA', label: 'Virtual', format: v => formatCurrency(v, true), color: 'text-blue-600' },
              { key: 'microEBITDA', label: 'Micro', format: v => formatCurrency(v, true), color: 'text-emerald-600' },
              { key: 'midSizedEBITDA', label: 'Mid-Sized', format: v => formatCurrency(v, true), color: 'text-amber-600' },
              { key: 'flagshipEBITDA', label: 'Flagship', format: v => formatCurrency(v, true), color: 'text-purple-600' },
              { key: 'totalEBITDA', label: 'Total', format: v => formatCurrency(v, true), bold: true },
              { key: 'ebitdaMargin', label: 'Margin', format: v => `${(v * 100).toFixed(1)}%` },
            ],
            cashflow: [
              { key: 'year', label: 'Year', format: v => v },
              { key: 'totalEBITDA', label: 'EBITDA', format: v => formatCurrency(v, true), color: 'text-green-600' },
              { key: 'totalCapex', label: 'CapEx', format: v => formatCurrency(v, true), color: 'text-red-600' },
              { key: 'netCashFlow', label: 'Net CF', format: v => formatCurrency(v, true) },
              { key: 'cumulativeCashFlow', label: 'Cumulative', format: v => formatCurrency(v, true), bold: true },
            ],
          };
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {sortedData.map((row, idx) => (
                    <tr key={row.year} className={`border-b hover:bg-gray-50 ${idx === 10 ? 'bg-blue-50' : ''}`}>
                      {cols.map(col => (
                        <td
                          key={col.key}
        ...
        function ValueCreationWaterfall({ model, params }) {
          const year10 = model.years[10];

          const waterfallData = [
            { name: 'Virtual EBITDA', value: year10.virtualEBITDA * params.ebitdaMultiple, fill: COLORS.virtual },
            { name: 'Micro EBITDA', value: year10.microEBITDA * params.ebitdaMultiple, fill: COLORS.micro },
            { name: 'Mid-Sized EBITDA', value: year10.midSizedEBITDA * params.ebitdaMultiple, fill: COLORS.midSized },
            { name: 'Flagship EBITDA', value: year10.flagshipEBITDA * params.ebitdaMultiple, fill: COLORS.flagship },
            { name: 'Property Value', value: (year10.midSizedSchools * 30000000) + (year10.flagshipSchools * 100000000), fill: '#6B7280' },
          ];
          ...
          return (
            <div>
              <h3 className="font-semibold text-gray-900 mb-4">Value Creation Breakdown</h3>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  {waterfallData.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className="font-bold">{formatCurrency(item.value, true)}</span>
                    </div>
                  ))}
                  <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg border-2 border-blue-300">
                    <span className="font-bold text-lg">Total Terminal Value</span>
                    <span className="font-bold text-2xl text-blue-700">{formatCurrency(model.summary.terminalValue, true)}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        function TSAScenarioModel() {
          ...
          const revenueMix = [
            { name: 'Virtual', value: model.summary.totalRevenue > 0 ? model.years[10].virtualRevenue : 0, color: COLORS.virtual },
            { name: 'Micro', value: model.years[10].microRevenue, color: COLORS.micro },
            { name: 'Mid-Sized', value: model.years[10].midSizedRevenue, color: COLORS.midSized },
            { name: 'Flagship', value: model.years[10].flagshipRevenue, color: COLORS.flagship },
          ];
          ...
          return (
            <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-4 md:p-6">
              <div className="max-w-7xl mx-auto">
                {/* Header */}
                <div className="mb-6">
                  <h1 className="text-3xl font-bold text-gray-900 mb-2">TSA / Strata Financial Model</h1>
                  <p className="text-gray-600">Interactive 10-year scenario modeling and analysis</p>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border p-4 mb-6">
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    >
                      Custom Parameters
                      <Icon name={showAdvanced ? 'chevron-up' : 'chevron-down'} size={18} />
                    </button>
                  </div>
                  ...
                  {showAdvanced && (
                    <div className="mt-6 pt-6 border-t grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div>
                        <h3 className="font-medium text-gray-900 mb-4">Growth Multipliers</h3>
                        <SliderInput
                      ...
                      <div>
                        <h3 className="font-medium text-gray-900 mb-4">Capital & Costs</h3>
                        <SliderInput
                      ...
                      <div>
                        <h3 className="font-medium text-gray-900 mb-4">Valuation</h3>
                        <SliderInput
                        ...
                        <div className="mt-6">
                          ...
                          >
                            ...
                            {isRunningMC ? 'Running...' : 'Run Monte Carlo (500 sims)'}
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border mb-6">
                  <div className="flex border-b px-4 overflow-x-auto">
                    <TabButton isActive={activeTab === 'overview'} onClick={() => setActiveTab('overview')}>
                      <span className="flex items-center gap-2">
                        <Icon name="bar-chart-3" size={16} /> Overview
                      </span>
                    </TabButton>
                    <TabButton isActive={activeTab === 'revenue'} onClick={() => setActiveTab('revenue')}>
                      <span className="flex items-center gap-2">
                        <Icon name="dollar-sign" size={16} /> Revenue
                      </span>
                    </TabButton>
                    <TabButton isActive={activeTab === 'cashflow'} onClick={() => setActiveTab('cashflow')}>
                      <span className="flex items-center gap-2">
                        <Icon name="activity" size={16} /> Cash Flow
                      </span>
                    </TabButton>
                    <TabButton isActive={activeTab === 'uniteconomics'} onClick={() => setActiveTab('uniteconomics')}>
                      <span className="flex items-center gap-2">
                        <Icon name="calculator" size={16} /> Unit Economics
                      </span>
                    </TabButton>
                    <TabButton isActive={activeTab === 'goalseek'} onClick={() => setActiveTab('goalseek')}>
                      <span className="flex items-center gap-2">
                        <Icon name="target" size={16} /> Goal Seek
                      </span>
                    </TabButton>
                    <TabButton isActive={activeTab === 'sensitivity'} onClick={() => setActiveTab('sensitivity')}>
                      <span className="flex items-center gap-2">
                        <Icon name="shuffle" size={16} /> Sensitivity
                      </span>
                    </TabButton>
                    <TabButton isActive={activeTab === 'data'} onClick={() => setActiveTab('data')}>
                      <span className="flex items-center gap-2">
                        <Icon name="table" size={16} /> Data Tables
                      </span>
                    </TabButton>
                  </div>
                  ...
                  <div className="p-6">
                    ...
                    {activeTab === 'overview' && (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                          <h3 className="font-semibold text-gray-900 mb-4">Student Enrollment by Tier</h3>
                          <ResponsiveContainer width="100%" height={300}>
                            <AreaChart data={studentData}>
                              <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                              <XAxis dataKey="year" tick={{ fontSize: 12 }} />
                              <YAxis tickFormatter={(v) => formatNumber(v, true)} tick={{ fontSize: 12 }} />
                              <Tooltip formatter={(v) => formatNumber(v)} />
                              <Legend />
                              <Area type="monotone" dataKey="Virtual" stackId="1" fill={COLORS.virtual} stroke={COLORS.virtual} />
                              <Area type="monotone" dataKey="Micro" stackId="1" fill={COLORS.micro} stroke={COLORS.micro} />
                              <Area type="monotone" dataKey="Mid-Sized" stackId="1" fill={COLORS.midSized} stroke={COLORS.midSized} />
                              <Area type="monotone" dataKey="Flagship" stackId="1" fill={COLORS.flagship} stroke={COLORS.flagship} />
                            </AreaChart>
                          </ResponsiveContainer>
                        </div>
                        <div>
                          <h3 className="font-semibold text-gray-900 mb-4">Year 10 Revenue Mix</h3>
                          <ResponsiveContainer width="100%" height={300}>
                            <PieChart>
                              <Pie
                                data={revenueMix}
                        ...
                        <div className="lg:col-span-2">
                          <h3 className="font-semibold text-gray-900 mb-4">Margin Progression</h3>
                          <ResponsiveContainer width="100%" height={250}>
                            <LineChart data={marginData}>
                              <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                              <XAxis dataKey="year" tick={{ fontSize: 12 }} />
                              <YAxis tickFormatter={(v) => `${v.toFixed(0)}%`} tick={{ fontSize: 12 }} domain={[0, 40]} />
                              <Tooltip formatter={(v) => `${v.toFixed(1)}%`} />
                              <Legend />
                              <Line type="monotone" dataKey="EBITDA Margin" stroke={COLORS.ebitda} strokeWidth={2} dot={false} />
                              <Line type="monotone" dataKey="Timeback %" stroke={COLORS.timeback} strokeWidth={2} dot={false} />
                              <ReferenceLine y={20} stroke="#9CA3AF" strokeDasharray="5 5" label={{ value: '20% Target', position: 'right', fontSize: 10 }} />
                            </LineChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
                    ...
                    {activeTab === 'revenue' && (
                      <div>
                        <h3 className="font-semibold text-gray-900 mb-4">Revenue by Tier ($M)</h3>
                        <ResponsiveContainer width="100%" height={400}>
                          <AreaChart data={revenueData}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                            <XAxis dataKey="year" tick={{ fontSize: 12 }} />
                            <YAxis tickFormatter={(v) => `$${v.toFixed(0)}M`} tick={{ fontSize: 12 }} />
                            <Tooltip formatter={(v) => formatCurrency(v * 1e6, true)} />
                            <Legend />
                            <Area type="monotone" dataKey="Virtual" stackId="1" fill={COLORS.virtual} stroke={COLORS.virtual} />
                            <Area type="monotone" dataKey="Micro" stackId="1" fill={COLORS.micro} stroke={COLORS.micro} />
                            <Area type="monotone" dataKey="Mid-Sized" stackId="1" fill={COLORS.midSized} stroke={COLORS.midSized} />
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b">
                                ...
                                <th className="text-right py-2 font-medium text-gray-600">Virtual</th>
                                <th className="text-right py-2 font-medium text-gray-600">Micro</th>
                                <th className="text-right py-2 font-medium text-gray-600">Mid-Sized</th>
                                <th className="text-right py-2 font-medium text-gray-600">Flagship</th>
                            ...
                            <tbody>
                              {model.years.map((y) => (
                                <tr key={y.year} className="border-b hover:bg-gray-50">
                                  ...
                                  <td className="text-right text-blue-600">{formatCurrency(y.virtualRevenue, true)}</td>
                                  <td className="text-right text-emerald-600">{formatCurrency(y.microRevenue, true)}</td>
                                  <td className="text-right text-amber-600">{formatCurrency(y.midSizedRevenue, true)}</td>
                                  <td className="text-right text-purple-600">{formatCurrency(y.flagshipRevenue, true)}</td>
                                  <td className="text-right font-bold">{formatCurrency(y.totalRevenue, true)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                    ...
                    {activeTab === 'cashflow' && (
                      <div>
                        <h3 className="font-semibold text-gray-900 mb-4">Cash Flow Waterfall ($M)</h3>
                        <ResponsiveContainer width="100%" height={400}>
                          <ComposedChart data={cashFlowData}>
                            ...
                            <Tooltip formatter={(v) => formatCurrency(v * 1e6, true)} />
                            <Legend />
                            <Bar dataKey="EBITDA" fill={COLORS.ebitda} />
                            <Bar dataKey="CapEx" fill={COLORS.capex} />
                            <Line type="monotone" dataKey="Cumulative" stroke="#1F2937" strokeWidth={2} dot={{ fill: '#1F2937' }} />
                        ...
                        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="bg-gray-50 rounded-lg p-4">
                            <h4 className="font-medium text-gray-700 mb-2">Cumulative CapEx</h4>
                            <p className="text-2xl font-bold text-gray-900">{formatCurrency(model.summary.cumulativeCapex, true)}</p>
                          </div>
                          <div className="bg-red-50 rounded-lg p-4">
                            <h4 className="font-medium text-red-700 mb-2">Peak Funding Requirement</h4>
                            <p className="text-2xl font-bold text-red-900">{formatCurrency(model.summary.peakFunding, true)}</p>
                          </div>
                          <div className="bg-green-50 rounded-lg p-4">
                            <h4 className="font-medium text-green-700 mb-2">Breakeven Year</h4>
                            <p className="text-2xl font-bold text-green-900">{model.summary.breakeven > 0 ? YEARS[model.summary.breakeven] : 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === 'uniteconomics' && (
                      <UnitEconomicsPanel model={model} />
                    )}
                    ...
                    {activeTab === 'goalseek' && (
                      <div className="space-y-6">
                        ...
                        />
                        <ValueCreationWaterfall model={model} params={params} />
                      </div>
                    )}
                    ...
                    {activeTab === 'sensitivity' && (
                      <div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          <div>
                            <h3 className="font-semibold text-gray-900 mb-4">Terminal Value vs Mid-Sized Growth</h3>
                            <ResponsiveContainer width="100%" height={300}>
                              <LineChart data={sensitivityData.growth}>
                                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                                <XAxis
                                  dataKey="multiplier"
                                  tickFormatter={(v) => `${(v * 100).toFixed(0)}%`}
                                  tick={{ fontSize: 12 }}
                                  label={{ value: 'Growth Rate', position: 'bottom', offset: -5 }}
                                />
                          ...
                          <div>
                            <h3 className="font-semibold text-gray-900 mb-4">Terminal Value vs EBITDA Multiple</h3>
                            <ResponsiveContainer width="100%" height={300}>
                              <LineChart data={sensitivityData.multiple}>
                                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                                <XAxis
                                  dataKey="multiplier"
                                  tickFormatter={(v) => `${(v * params.ebitdaMultiple).toFixed(1)}x`}
                                  tick={{ fontSize: 12 }}
                                  label={{ value: 'EBITDA Multiple', position: 'bottom', offset: -5 }}
                                />
                                <YAxis tickFormatter={(v) => formatCurrency(v, true)} tick={{ fontSize: 12 }} />
                                <Tooltip
                                  formatter={(v) => formatCurrency(v, true)}
                        ...
                        {monteCarloResults && (
                          <div className="mt-8 pt-6 border-t">
                            <h3 className="font-semibold text-gray-900 mb-4">Monte Carlo Simulation Results (500 iterations)</h3>
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                              <div className="bg-red-50 rounded-lg p-4 text-center">
                                <p className="text-sm text-red-700 mb-1">P10 (Pessimistic)</p>
                                <p className="text-lg font-bold text-red-900">{formatCurrency(monteCarloResults.p10.terminalValue, true)}</p>
                              </div>
                              <div className="bg-amber-50 rounded-lg p-4 text-center">
                                <p className="text-sm text-amber-700 mb-1">P25</p>
                                <p className="text-lg font-bold text-amber-900">{formatCurrency(monteCarloResults.p25.terminalValue, true)}</p>
                              </div>
                              <div className="bg-green-50 rounded-lg p-4 text-center">
                                <p className="text-sm text-green-700 mb-1">P50 (Median)</p>
                                <p className="text-lg font-bold text-green-900">{formatCurrency(monteCarloResults.p50.terminalValue, true)}</p>
                              </div>
                              <div className="bg-blue-50 rounded-lg p-4 text-center">
                                <p className="text-sm text-blue-700 mb-1">P75</p>
                                <p className="text-lg font-bold text-blue-900">{formatCurrency(monteCarloResults.p75.terminalValue, true)}</p>
                              </div>
                              <div className="bg-purple-50 rounded-lg p-4 text-center">
                                <p className="text-sm text-purple-700 mb-1">P90 (Optimistic)</p>
                                <p className="text-lg font-bold text-purple-900">{formatCurrency(monteCarloResults.p90.terminalValue, true)}</p>
                              </div>
                            </div>
                            <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                              <p className="text-sm text-gray-600">
                                <strong>Expected Value:</strong> {formatCurrency(monteCarloResults.mean.terminalValue, true)} |
                                <strong className="ml-4">Range:</strong> {formatCurrency(monteCarloResults.p10.terminalValue, true)} to {formatCurrency(monteCarloResults.p90.terminalValue, true)}
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    {activeTab === 'data' && (
                      <div>
                        <div className="flex gap-2 mb-6 flex-wrap">
                          {['students', 'revenue', 'ebitda', 'cashflow'].map(view => (
                            <button
                              key={view}
                        ...
                        <DataTableView model={model} viewMode={dataTableView} />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="text-center text-sm text-gray-500 mt-8">
                  <p>TSA Financial Scenario Model • Built with React & Recharts</p>
                </div>
              </div>
            </div>
          );
